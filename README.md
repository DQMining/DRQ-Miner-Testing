...
WALLET=crb1YOURADDRESS.worker ./mine-nm.sh
```

Wallet must start with **`crb1`**. No `nano` on Userland? Use `WALLET=... ./mine-nm.sh` or `sed -i 's/^WALLET=.*/WALLET=crb1....worker/' mine-nm.sh`.

Or one line:

```bash
./drqminer -a nm/1 -o us-east.hashmonkeys.cloud:12427 -u crb1YOURADDRESS.worker -p x -t 8
```
