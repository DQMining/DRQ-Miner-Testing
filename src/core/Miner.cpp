...
#   ifdef XMRIG_ALGO_VERUSHASH
    void warnVerusHashSetup() const
    {
        const Config *cfg = controller->config();
        const bool cpuOff = !cfg->cpu().isEnabled();
        bool oclOn        = false;
        bool cudaOn       = false;

#       ifdef XMRIG_FEATURE_OPENCL
        oclOn = cfg->cl().isEnabled();
#       endif
#       ifdef XMRIG_FEATURE_CUDA
        cudaOn = cfg->cuda().isEnabled();
#       endif

        if (oclOn) {
            LOG_WARN("%s " YELLOW_BOLD("verushash") "/" YELLOW_BOLD("VRSC") ": OpenCL threads run " RED_BOLD("CPU") " VerusHash (no OpenCL kernel). See doc/GPU_VERUS_NATIVE.md.",
                       Tags::miner());
        }
#       ifndef XMRIG_FEATURE_CUDA_VERUS
        if (cudaOn) {
            LOG_WARN("%s " YELLOW_BOLD("verushash") "/" YELLOW_BOLD("VRSC") ": CUDA threads run " RED_BOLD("CPU") " VerusHash — embed CUDA toolkit and rebuild for native GPU, or use ccminer (verus2.2gpu). See doc/GPU_VERUS_NATIVE.md.",
                       Tags::miner());
        }
#       endif

        if (cpuOff && !oclOn && !cudaOn) {
            LOG_ERR("%s " RED_BOLD("No backend enabled") " (cpu/opencl/cuda off) — enable " WHITE_BOLD("CPU") ", " WHITE_BOLD("OpenCL") ", or " WHITE_BOLD("CUDA") " in config (e.g. " WHITE_BOLD("--no-cpu") " requires a GPU backend).",
                    Tags::miner());
        }
    }
#   endif
