...
alignas(32) uint16_t CodeLUT_16[257] = {};

static std::once_flag s_lutOnce;

static void build_lut_16()
{
    for (int i = 0; i < 256; ++i) {
        uint16_t t = 0;
        const uint32_t opcode = CodeLUT[i];
        for (int j = 3; j >= 0; --j) {
            const uint8_t insn = static_cast<uint8_t>((opcode >> (j << 3)) & 0xFF);
            t |= static_cast<uint16_t>(insn << (j << 2));
        }
        CodeLUT_16[i] = t;
    }
}
...
void init_wolf_lut()
{
    std::call_once(s_lutOnce, build_lut_16);
}

uint8_t wolf_branch(uint8_t val, uint8_t pos2val, uint32_t opcode)
{
    for (int i = 3; i >= 0; --i) {
        const uint8_t insn = static_cast<uint8_t>((opcode >> (i << 3)) & 0xFF);
        switch (insn) {
        case 0:  val = static_cast<uint8_t>(val + val); break;
        case 1:  val = static_cast<uint8_t>(val - (val ^ 97)); break;
        case 2:  val = static_cast<uint8_t>(val * val); break;
        case 3:  val ^= pos2val; break;
        case 4:  val = static_cast<uint8_t>(~val); break;
        case 5:  val &= pos2val; break;
        case 6:  val = static_cast<uint8_t>(val << (val & 3)); break;
        case 7:  val = static_cast<uint8_t>(val >> (val & 3)); break;
        case 8:
            ...
            break;
        case 9:  val ^= static_cast<uint8_t>(popcount8(val)); break;
        case 10: val = rotl8(val, val); break;
