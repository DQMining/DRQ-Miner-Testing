...
bool mine_verus_v2(CBlockHeader &bh, CVerusHashV2bWriter &vhw, uint256 &finalHash, uint256 &target, uint64_t start, uint64_t *count)
{
	...
	for (i = start; i < end; i++)
	{
		*extraPtr = i;
        ...
        acc = _mm_xor_si128(acc, _mm_cvtsi32_si128(0x00010000));
...
__m128i __verusclmulwithoutreduction64alignedrepeat(__m128i *randomsource, const __m128i buf[4], uint64_t keyMask, __m128i **pMoveScratch)
{
    ...
    for (int64_t i = 0; i < 32; i++)
    {
        ...
        *(pMoveScratch++) = prandex;        
        ...
        pbuf = buf + (selector & 3);
        const __m128i * const pbufAlt = (selector & 1) ? (pbuf - 1) : (pbuf + 1);
        ...
        switch (selector & 0x1c)
        {
            case 0:
            {
                ...
                const __m128i temp2 = _mm_load_si128(pbufAlt);
            ...
            case 4:
            {
                ...
                const __m128i temp22 = _mm_load_si128(pbufAlt);
                const __m128i add12 = _mm_xor_si128(temp12, temp22);
            ...
            case 0x10:
            {
                ...
                __m128i temp1 = _mm_load_si128(pbufAlt);
            ...
            case 0x18:
            {
                const __m128i temp1 = _mm_load_si128(pbufAlt);
        ...
        *(pMoveScratch++) = prandex;        
        ...
        switch (selector & 0x1c)
        {
            ...
            case 0x1c:
            {
                ...
                acc = _mm_xor_si128(tempa3, acc);
                const __m128i temp4 = _mm_load_si128(pbufAlt); 
