/* NVIDIA Verus GPU kernel — from monkins1010/ccminer branch verus2.2gpu (GPLv3).
 * miner.h omitted (jansson/curl); cuda_helper replaced with minimal host macros. */
#include "crypto/verushash/cuda/verus_cuda_max_gpus.h"
#include "crypto/verushash/cuda/cuda_helper_compat.h"
...
__global__ void verus_gpu_hash(const uint32_t threads, const uint32_t startNonce, uint32_t * __restrict__ resNonce,
	uint128m * __restrict__ d_key_input, uint8_t version, uint32_t slot_mask);
...
/** Per-thread slot mask (slots-1); must match cudaMalloc size for d_long_keys[thr_id]. */
static uint32_t h_verus_slot_mask[MAX_GPUS];

static __host__ uint32_t verus_next_pow2_ge(uint32_t v)
{
	if (v <= 1u) {
		return 1u;
	}
...
__host__
bool verus_init(int thr_id, uint32_t throughput)
{
	if (thr_id < 0 || thr_id >= MAX_GPUS) {
		return false;
	}
	/* Re-init without leaking device pointers (e.g. worker restart). */
	if (d_long_keys[thr_id] != nullptr) {
		cudaFree(d_long_keys[thr_id]);
		d_long_keys[thr_id] = nullptr;
	}
	if (d_nonces[thr_id] != nullptr) {
		cudaFree(d_nonces[thr_id]);
		d_nonces[thr_id] = nullptr;
	}
	h_verus_slot_mask[thr_id] = 0u;

	uint32_t slots = verus_next_pow2_ge(throughput ? throughput : 1u);
	if (slots > TOTAL_MAX) {
		slots = TOTAL_MAX;
	}
	const size_t alloc_bytes = static_cast<size_t>(slots) * static_cast<size_t>(VERUS_KEY_SIZE);

	cudaError_t err = cudaMalloc(&d_nonces[thr_id], sizeof(uint32_t));
	if (err != cudaSuccess) {
		fprintf(stderr, "verus_init: cudaMalloc d_nonces: %s\n", cudaGetErrorString(err));
		return false;
	}
	err = cudaMalloc(&d_long_keys[thr_id], alloc_bytes);
	if (err != cudaSuccess) {
		fprintf(stderr, "verus_init: cudaMalloc d_long_keys (%zu MiB): %s\n",
			static_cast<size_t>(alloc_bytes / (1024u * 1024u)),
			cudaGetErrorString(err));
		cudaFree(d_nonces[thr_id]);
		d_nonces[thr_id] = nullptr;
		return false;
	}
	h_verus_slot_mask[thr_id] = slots - 1u;
	return true;
}
...
void verus_setBlock(uint8_t *blockf, uint32_t *pTargetIn, uint8_t *lkey, int thr_id, uint32_t throughput)
{
	...
	CUDA_SAFE_CALL(cudaMemcpyToSymbol(vkey, lkey, VERUS_KEY_SIZE * sizeof(uint8_t), 0, cudaMemcpyHostToDevice));
...
void verus_hash(int thr_id, uint32_t threads, uint32_t startNonce, uint32_t *resNonces, uint8_t version)
{
	...
	const uint32_t slot_mask = (thr_id >= 0 && thr_id < MAX_GPUS) ? h_verus_slot_mask[thr_id] : (TOTAL_MAX - 1u);
	verus_gpu_hash << <grid, block >> >(threads, startNonce, d_nonces[thr_id], d_long_keys[thr_id], version, slot_mask);
...
__device__ __forceinline__ uint128m _mm_shuffle_epi8_emu(uint2 b)
{
	...
	const uint2 W = make_uint2(b.x & Q.x, b.y & Q.y);
	...
	uint64_t selector, uint128m &acc, uint128m *randomsource, uint32_t prand_idx)
{
	...
	do {
		...
		if (!loop_c && (rounds != (uint64_t)0xffffffffffffffff))
		{
			...
			uint4 add1 = _mm_xor_si128_emu(onekey, temp2);
	...
	uint128m * __restrict__ d_key_input, uint8_t version, uint32_t slot_mask)
{
	...
	for (int i = 0; i < 512; i++) {
		...
		d_key_input[(VERUS_KEY_SIZE128 * (thread & slot_mask)) + ((threadIdx.x + i) & 511)] = sharedMemory3[((threadIdx.x + i) & 511)];
	}
	int b = threadIdx.x % 40;
	for (int i = 0; i < 40; i++) {

		d_key_input[((VERUS_KEY_SIZE128 * (thread & slot_mask))) + 512 + ((b) % 40)] = sharedMemory3[512 + ((b) % 40)]; b++;
	}
	s[2].x = nounce;
	s[0] = _mm_xor_si128_emu(s[0], s[2]);
	s[1] = _mm_xor_si128_emu(s[1], s[3]);


	uint2 acc = __verusclmulwithoutreduction64alignedrepeatgpu(&d_key_input[(VERUS_KEY_SIZE128 * (thread & slot_mask))], s, sharedMemory1[0], version);
	...
	uint32_t hash = haraka512_port_keyed2222(s, (&d_key_input[(VERUS_KEY_SIZE128 * (thread & slot_mask))] + acc.x), sharedMemory1[0]);
	if (hash < ptarget[7]) 
	{
		resNonce[0] = nounce;
	}
...
/* Stable C entry points for the xmrig CUDA worker (host linkage). */
extern "C" int xmrig_verus_cuda_init(int thr_id, uint32_t throughput)
{
	return verus_init(thr_id, throughput) ? 0 : -1;
}

extern "C" void xmrig_verus_cuda_set_block(uint8_t *blockf, uint32_t *pTargetIn, uint8_t *lkey, int thr_id, uint32_t throughput)
{
	verus_setBlock(blockf, pTargetIn, lkey, thr_id, throughput);
}

extern "C" void xmrig_verus_cuda_hash(int thr_id, uint32_t threads, uint32_t startNonce, uint32_t *resNonces, uint8_t version)
{
	verus_hash(thr_id, threads, startNonce, resNonces, version);
}

extern "C" void xmrig_verus_cuda_cleanup(int thr_id)
{
	if (thr_id < 0 || thr_id >= MAX_GPUS) {
		return;
	}
