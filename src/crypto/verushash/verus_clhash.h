...
#include "crypto/verushash/compat/sse2neon.h"
#else
#include "crypto/verushash/sse2neon.h"
#endif
...
#if defined(_MSC_VER)
// GCC-compatible CPUID helpers used by the Verus reference code.
#ifndef __attribute__
#define __attribute__(x)
#endif
#ifndef bit_PCLMUL
#define bit_PCLMUL (1U << 1)
#endif
#ifndef bit_AES
...
static inline int __get_cpuid(unsigned int leaf, unsigned int *eax, unsigned int *ebx, unsigned int *ecx, unsigned int *edx)
{
    int regs[4];
...
inline std::string LEToHex(const T &pt)
{
    ...
    {
        std::snprintf(hex, sizeof(hex), "%02x", *((unsigned char *)&pt + l));
        ss << hex;
    }
    return ss.str();
}
...
inline std::string HexBytes(const unsigned char *buf, int size)
{
    ...
    {
        std::snprintf(hex, sizeof(hex), "%02x", *(buf + l));
        ss << hex;
    }
    return ss.str();
}
...
struct verusclhasher {
    ...
    inline uint64_t hashWithFixup(const unsigned char buf[64], void **outKey)
    {
        unsigned char *key = (unsigned char *)verusclhasher_key.get();
        verusclhash_descr *desc = (verusclhash_descr *)verusclhasher_descr.get();
        __m128i **pMoveScratch = getpmovescratch(key + desc->keySizeInBytes);
        __m128i **fixList = pMoveScratch;

        // Restore moved key blocks from refresh region before hashing.
        const uint32_t ofs = desc->keySizeInBytes >> 4;
        for (__m128i *pfixup = *fixList; pfixup; pfixup = *++fixList) {
            *pfixup = *(pfixup + ofs);
        }
        ...
        return (*verusclhashfunction)(key, buf, keyMask, pMoveScratch);
    }
