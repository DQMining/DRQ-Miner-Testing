...
#   ifdef XMRIG_ALGO_VERUSHASH
        ...
        for (uint32_t nonce : bundle.nonces) {
            Job j = bundle.job;
            writeUnaligned(j.nonce(), nonce);
            alignas(16) uint8_t hash[32]{};
            verushash::single_hash_fast(j.blob(), j.size(), hash, 0);

            if (j.hasVerusTarget() && j.isVerusTargetReached(hash)) {
                results.emplace_back(j, nonce, hash);
            }
            else if (!j.hasVerusTarget() && (*reinterpret_cast<uint64_t *>(hash + 24) < j.target())) {
                results.emplace_back(j, nonce, hash);
            }
            else {
                LOG_ERR("%s " RED_S "GPU #%u COMPUTE ERROR", backend_tag(bundle.job.backend()), bundle.device_index);
...
public:
    ...
    inline void submit(const Job &job, uint64_t nonce, const uint8_t *result, const uint8_t *miner_signature)
    {
        std::lock_guard<SpinLock> lock(m_mutex);
        m_results.emplace_back(job, nonce, result, nullptr, nullptr, miner_signature);
...
#   if defined(XMRIG_FEATURE_OPENCL) || defined(XMRIG_FEATURE_CUDA)
    inline void submit()
    {
        ...
        m_mutex.unlock();
        m_asyncPending.store(false, std::memory_order_release);
...
#   endif
    ...
    inline void notifyAsync()
    {
        if (!m_asyncPending.exchange(true, std::memory_order_acq_rel)) {
            m_async->send();
        }
    }
...
void xmrig::JobResults::submit(const Job &job, uint64_t nonce, const uint8_t *result)
{
    ...
    if (handler) {
        handler->submit(job, nonce, result, nullptr);
    }
}
...
void xmrig::JobResults::submit(const Job& job, uint64_t nonce, const uint8_t* result, const uint8_t* miner_signature)
{
    ...
    if (handler) {
        handler->submit(job, nonce, result, miner_signature);
    }
}
