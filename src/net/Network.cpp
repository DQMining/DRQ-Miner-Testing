...
static inline bool quietAcceptedResults()
{
    static const bool quiet = []() {
        const char *env = std::getenv("XMRIG_QUIET_ACCEPTED");
        return env && (env[0] == '1');
    }();
...
static inline bool quietJobAnnouncements()
{
    static const bool quiet = []() {
        const char *env = std::getenv("XMRIG_QUIET_JOBS");
        return env && (env[0] == '1');
    }();
...
void xmrig::Network::onVerifyAlgorithm(IStrategy *, const IClient *, const Algorithm &algorithm, bool *ok)
{
    if (!m_controller->miner()->isEnabled(algorithm)) {
        *ok = false;
        ...
        LOG_ERR("%s " RED("miner has no working backend for algorithm ") RED_BOLD("\"%s\"") RED(" (e.g. --no-cpu needs CUDA/OpenCL with threads for that algo, or rebuild with embedded CUDA Verus)."),
                Tags::network(), algorithm.name());
