...
#   if defined(XMRIG_FEATURE_TLS) && defined(XMRIG_ALGO_ASTROBWT)
    if ((m_pool.zmq_port() >= 0) || m_pool.isWSS()) {
        if (m_pool.isWSS()) {
            if (m_dns || m_wss.m_socket) {
                return;
            }
            if (!isQuiet()) {
                LOG_INFO("%s " CYAN("WSS connecting to ") CYAN_BOLD("%s:%u"), tag(), m_pool.host().data(), m_pool.port());
            }
            m_wss.m_awaitRetry = false;
            m_wss.m_skipCloseRetry = false;
            constexpr uint64_t kWssConnectTimeoutMs = 20000;
            m_timer->start(kWssConnectTimeoutMs, 0);
        }
...
#   if defined(XMRIG_FEATURE_TLS) && defined(XMRIG_ALGO_ASTROBWT)
    if (m_pool.isWSS()) {
        if (m_state == ConnectingState) {
            if (m_wss.m_awaitRetry) {
                m_wss.m_awaitRetry = false;
                connect();
            }
            else {
                if (!isQuiet()) {
                    LOG_WARN("%s " YELLOW_BOLD("WSS connect timeout, retrying..."), tag());
                }
                m_dns.reset();
                if (m_wss.m_socket) {
                    m_wss.m_skipCloseRetry = true;
                    WSSClose();
                }
                m_wss.m_awaitRetry = true;
                m_timer->stop();
                m_timer->start(m_retryPause, 0);
                if (!m_wss.m_socket && !isQuiet()) {
                    LOG_INFO("%s " YELLOW_BOLD("reconnecting in %" PRIu64 "s..."), tag(), m_retryPause / 1000);
                }
            }
        }
        else if (m_state == ConnectedState && m_wss.m_socket && !m_wss.m_handshake) {
            WSSWriteFrame(0x9, nullptr, 0);
        }
        return;
    }
#   endif
...
#           endif
            ...
            else if (m_pool.isWSS()) {
                constexpr uint64_t kWssPingMs = 10000;
                m_timer->start(kWssPingMs, kWssPingMs);
            }
            else {
...
#       endif
        const bool skipRetry = client->m_wss.m_skipCloseRetry;
...
void xmrig::DaemonClient::WSSConnected()
{
    ...
    // Rabid and other pool WSS endpoints commonly use self-signed TLS (same as TNN verify_none).
    SSL_CTX_set_verify(m_wss.m_ctx, SSL_VERIFY_NONE, nullptr);

    m_wss.m_ssl = SSL_new(m_wss.m_ctx);
    SSL_set_connect_state(m_wss.m_ssl);
    SSL_set_bio(m_wss.m_ssl, m_wss.m_read, m_wss.m_write);

    const char* sni = m_pool.host().data();
    if (sni && *sni) {
        SSL_set_tlsext_host_name(m_wss.m_ssl, sni);
    }
...
bool xmrig::DaemonClient::WSSWriteFrame(uint8_t opcode, const void* payload, size_t size)
{
    if (!m_wss.m_socket || m_wss.m_handshake) {
        return false;
    }
    ...
    if (h.payload_len == 126) {
        SSL_write(m_wss.m_ssl, size_buf, 2);
    }
    if (h.payload_len == 127) {
        SSL_write(m_wss.m_ssl, size_buf, 8);
    }
    ...
    if (h.mask) {
        ...
        SSL_write(m_wss.m_ssl, mask, 4);
    }

    if (size > 0 && payload) {
        uint8_t masked[512];
        if (size > sizeof(masked)) {
            return false;
        }
        const auto* p = static_cast<const uint8_t*>(payload);
        for (size_t i = 0; i < size; ++i) {
            masked[i] = static_cast<uint8_t>(p[i] ^ mask[i % 4]);
        }
        SSL_write(m_wss.m_ssl, masked, static_cast<int>(size));
    }
...
#   endif
        ...
        // HTTP upgrade request (pre-WebSocket): raw TLS write, no WS framing.
        if (m_wss.m_handshake) {
            SSL_write(m_wss.m_ssl, data, static_cast<int>(size));
        }
        else if (!WSSWriteFrame(1, data, size)) {
            return false;
        }
    }
...
void xmrig::DaemonClient::WSSRead(ssize_t nread, const uv_buf_t* read_buf)
{
    if (nread <= 0) {
        if (nread == UV_EOF) {
            LOG_WARN("%s " YELLOW_BOLD("WSS disconnected by pool, reconnecting..."), tag());
        }
        else {
            LOG_ERR("%s " RED("WSS read failed: ") RED_BOLD("\"%s\""), tag(), uv_strerror(static_cast<int>(nread)));
        }
        WSSClose();
        return;
    }
    ...
    if (!SSL_is_init_finished(m_wss.m_ssl)) {
        ...
        if ((rc < 0) && (SSL_get_error(m_wss.m_ssl, rc) == SSL_ERROR_WANT_READ)) {
            ...
            if (!isQuiet()) {
                LOG_ERR("%s " RED("WSS TLS handshake failed (rc=%d, err=%d)"), tag(), rc, SSL_get_error(m_wss.m_ssl, rc));
            }
            WSSClose();
            return;
        }
            ...
            char hostPort[256];
            const int hostLen = snprintf(hostPort, sizeof(hostPort), "%s:%u", m_pool.host().data(), m_pool.port());
            if (hostLen <= 0 || hostLen >= static_cast<int>(sizeof(hostPort))) {
                WSSClose();
                return;
            }

            const int n = snprintf(m_wss.m_buf, sizeof(m_wss.m_buf), kWSSLogin, m_pool.user().data(), hostPort, key);
    ...
    while ((n = SSL_read(m_wss.m_ssl, m_wss.m_buf, sizeof(m_wss.m_buf))) > 0) {
            ...
            if (len >= 4) {
                for (size_t k = 0; k <= len - 4; ++k) {
                    if (memcmp(m_wss.m_data.data() + k, "\r\n\r\n", 4) == 0) {
                        ...
                        if (!isQuiet()) {
                            LOG_INFO("%s " GREEN_BOLD("WSS connected to ") GREEN_BOLD("%s"), tag(), m_pool.host().data());
                        }
                        break;
                    }
                }
        ...
        for (uint64_t i = 0; i < len; ++i) {
            payload.push_back(static_cast<char>(p[i] ^ mask_key[i % 4]));
        }
        p += len;

        m_wss.m_data.erase(m_wss.m_data.begin(), m_wss.m_data.begin() + static_cast<std::vector<char>::difference_type>(p - p0));

        const uint8_t opcode = h->opcode;

        // WebSocket control frames (RFC 6455)
        if (opcode == 0x9) {
            if (payload.empty()) {
                WSSWriteFrame(0xA, nullptr, 0);
            }
            else {
                WSSWriteFrame(0xA, payload.data(), payload.size());
            }
            continue;
        }
        if (opcode == 0xA) {
        ...
        if (opcode == 0x8) {
            LOG_WARN("%s " YELLOW_BOLD("WSS close frame received, reconnecting..."), tag());
            WSSClose();
            return;
        }
        if (opcode != 0x1) {
        ...
        if (h->fin) {
            m_wss.m_message = std::move(payload);
            if (!m_wss.m_message.empty() && m_wss.m_message.back() == '\n') {
