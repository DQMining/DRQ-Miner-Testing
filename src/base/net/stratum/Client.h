...
private:
    ...
    bool m_keepAliveRejected    = false;
    uintptr_t m_key             = 0;
...
template<> inline bool Client::has<Client::EXT_KEEPALIVE>() const noexcept
{
    // Be strict: only use keepalive when the pool explicitly advertises support.
    // This avoids repeated "Unsupported request keepalived" turbulence on pools
    // that reject keepalive despite user-side keepalive settings.
    return !m_keepAliveRejected && m_extensions.test(EXT_KEEPALIVE);
}
