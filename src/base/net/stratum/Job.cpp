...
bool xmrig::Job::setVerusTarget(const char *target)
{
    m_hasVerusTarget = false;
    ...
    uint8_t beTarget[32]{ 0 };
    if (!Cvt::fromHex(beTarget, sizeof(beTarget), target, size)) {
        return false;
    }

    // Pool target is sent as big-endian hex, hash comparisons are little-endian numeric.
    for (size_t i = 0; i < sizeof(m_verusTarget); ++i) {
        m_verusTarget[i] = beTarget[31 - i];
    }
    for (size_t i = 0; i < 4; ++i) {
        m_verusTarget64[i] = readUnaligned(reinterpret_cast<const uint64_t *>(m_verusTarget + (i * sizeof(uint64_t))));
    }

    // Keep legacy 64-bit target/diff populated for worker-side quick checks.
    char hi64[17]{ 0 };
    memcpy(hi64, target, 16);
    const uint64_t hi = strtoull(hi64, nullptr, 16);
    if (hi > 0) {
        m_target = hi;
        m_diff   = toDiff(m_target);
    }

    m_hasVerusTarget = true;
    return true;
}
...
size_t xmrig::Job::nonceOffset() const
{
    ...
    case Algorithm::VERUSHASH_FAMILY:
        // Verus solution tail on this pool is byte-shifted:
        // [pad(1 byte)][nonceSpace(15 bytes)] and counter is last 4 bytes of nonceSpace.
        if (m_size >= 7) {
            // SRBMiner-compatible layout observed on this pool:
            // nonceSpace = [ex1(4)][round(4)][counter(4)][reserved(3)].
            // Counter starts 7 bytes from the end of the full blob.
            return m_size - 7;
        }
        break;
...
void xmrig::Job::copy(const Job &other)
{
    ...
    m_verusNTime = other.m_verusNTime;
    m_verusSolution = other.m_verusSolution;
    m_verusSolutionSize = other.m_verusSolutionSize;
    m_hasVerusTarget = other.m_hasVerusTarget;

    memcpy(m_blob, other.m_blob, sizeof(m_blob));
    memcpy(m_verusTarget, other.m_verusTarget, sizeof(m_verusTarget));
    memcpy(m_verusTarget64, other.m_verusTarget64, sizeof(m_verusTarget64));
...
void xmrig::Job::move(Job &&other)
{
    ...
    m_verusNTime = std::move(other.m_verusNTime);
    m_verusSolution = std::move(other.m_verusSolution);
    m_verusSolutionSize = other.m_verusSolutionSize;
    m_hasVerusTarget = other.m_hasVerusTarget;
    ...
    other.m_verusSolutionSize = 0;
    other.m_hasVerusTarget = false;
