...
static inline bool containsIgnoreCase(const char *haystack, const char *needle)
{
    ...
    for (const char *h = haystack; *h; ++h) {
        ...
        while (*h2 && *n2 && (std::tolower(static_cast<unsigned char>(*h2)) == std::tolower(static_cast<unsigned char>(*n2)))) {
            ++h2;
            ++n2;
        }
...
static inline bool isUnsupportedKeepaliveError(const char *message)
{
    return containsIgnoreCase(message, "unsupported request keepalived");
}
...
bool xmrig::Client::write(const uv_buf_t &buf)
{
    ...
    const size_t written = rc > 0 ? static_cast<size_t>(rc) : 0;
    if (written >= buf.len) {
        return true;
    }

    const size_t pending = buf.len - written;
    auto *req = new uv_write_t;
    auto *outData = new char[pending];
    memcpy(outData, buf.base + written, pending);

    uv_buf_t out = uv_buf_init(outData, static_cast<unsigned int>(pending));
    req->data = outData;

    const int wrc = uv_write(req, stream(), &out, 1, onWrite);
    if (wrc == 0) {
        return true;
    }
...
{
    auto *data = static_cast<char *>(req->data);
    delete[] data;
    req->data = nullptr;

    Client *client = getClient(req->handle->data);
    if (status < 0 && client) {
        if (!client->isQuiet()) {
...
void xmrig::Client::parse(char *line, size_t len)
{
    ...
    if (error.IsObject()) {
        ...
        // Some pools advertise keepalive extension but reject keepalived requests.
        // Disable keepalive for this connection after first explicit rejection.
        if (isUnsupportedKeepaliveError(message)) {
            m_keepAliveRejected = true;
            setExtension(EXT_KEEPALIVE, false);
            m_keepAlive = 0;
        }
