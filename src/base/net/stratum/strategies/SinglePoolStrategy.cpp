...
void xmrig::SinglePoolStrategy::onClose(IClient *client, int failures)
{
    if (!isActive()) {
        return;
    }

    // Daemon WSS pools reconnect automatically; keep mining active while retrying.
    if (client->pool().mode() == Pool::MODE_DAEMON && client->pool().isWSS() && failures > 0 && failures < 1000) {
        return;
    }
