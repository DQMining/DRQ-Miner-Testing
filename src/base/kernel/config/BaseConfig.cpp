...
#   ifdef XMRIG_FEATURE_BENCHMARK
    return m_pools.active() > 0 || m_pools.isBenchmark();
#   else
...
#   if defined(XMRIG_FEATURE_CUDA) && defined(XMRIG_ALGO_VERUSHASH)
#   ifdef XMRIG_FEATURE_CUDA_VERUS
    Log::print(GREEN_BOLD(" * ") WHITE_BOLD("%-13s") GREEN_BOLD("yes") BLACK_BOLD(" (Verus on NVIDIA without xmrig-cuda.dll)"), "CUDA VERUS");
#   else
    Log::print(GREEN_BOLD(" * ") WHITE_BOLD("%-13s") YELLOW_BOLD("no") YELLOW(" — for GPU Verus: install ") WHITE_BOLD("CUDA Toolkit") YELLOW(", CMake must find ") WHITE_BOLD("nvcc") YELLOW(", rebuild; or add ") WHITE_BOLD("xmrig-cuda.dll"), "CUDA VERUS");
#   endif
#   endif
}
