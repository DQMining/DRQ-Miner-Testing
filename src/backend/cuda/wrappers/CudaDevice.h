...
public:
    ...
    inline bool isValid() const                     { return m_ctx != nullptr || m_nativeRuntime; }
...
#   ifdef XMRIG_FEATURE_CUDA_VERUS
    /** When xmrig-cuda is not loaded, enumerate GPUs via CUDA runtime (embedded Verus kernel in main binary). */
    static std::vector<CudaDevice> enumerateEmbeddedCudaDevices(int32_t bfactor, int32_t bsleep);
#   endif

private:
#   ifdef XMRIG_FEATURE_CUDA_VERUS
    struct NativeCudaProbeTag
    {
    };
    CudaDevice(NativeCudaProbeTag, uint32_t index, int32_t bfactor, int32_t bsleep);
#   endif
    ...
    bool m_nativeRuntime            = false;
    uint32_t m_nativeSmx            = 0;
    uint32_t m_nativeClockKhz       = 0;
    uint32_t m_nativeMemClockKhz     = 0;
    uint32_t m_nativeArchMajor      = 0;
    uint32_t m_nativeArchMinor      = 0;
    size_t m_nativeTotalMem         = 0;
