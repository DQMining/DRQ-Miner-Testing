...
static bool     cudaPluginLoaded = false;
...
/** Windows: mismatched xmrig-cuda.dll often faults inside plugin init(); catch SEH so embedded CUDA Verus can run without the plugin. */
#   ifdef _WIN32
static bool cudaPluginInitSafe(init_t fn)
{
    if (!fn) {
        return false;
    }
...
static void cudaPluginClearSymbols() noexcept
{
    pVersion         = nullptr;
    pAlloc           = nullptr;
    pCnHash          = nullptr;
    pDeviceCount     = nullptr;
    pDeviceInfo      = nullptr;
    pDeviceInfo_v2   = nullptr;
    pDeviceInit      = nullptr;
    pDeviceInt       = nullptr;
    pDeviceName      = nullptr;
    pDeviceUint      = nullptr;
    pDeviceUlong     = nullptr;
    pInit            = nullptr;
    pLastError       = nullptr;
    pPluginVersion   = nullptr;
    pRelease         = nullptr;
    pRxHash          = nullptr;
    pRxPrepare       = nullptr;
    pRxUpdateDataset = nullptr;
    pKawPowHash      = nullptr;
    pKawPowPrepare_v2 = nullptr;
    pKawPowStopHash  = nullptr;
    pSetJob          = nullptr;
    pSetJob_v2       = nullptr;
}


static void unloadCudaPlugin() noexcept
{
    cudaPluginClearSymbols();
    if (!cudaPluginLoaded) {
        return;
    }
    uv_dlclose(&cudaLib);
    cudaPluginLoaded = false;
}
...
#       ifdef XMRIG_FEATURE_CUDA_VERUS
        /* Embedded Verus uses CUDA runtime in the main binary. Loading xmrig-cuda.dll alongside it
         * causes STATUS_HEAP_CORRUPTION (0xC0000374) on Windows when both runtimes are active. */
        if (fileName == nullptr || m_loader == defaultLoader) {
            m_error = "embedded Verus CUDA (xmrig-cuda plugin skipped)";
            return false;
        }
#       endif
...
{
    if (ctx == nullptr || pRelease == nullptr) {
        return;
    }
    pRelease(ctx);
}
...
#   ifdef _WIN32
    if (!cudaPluginInitSafe(pInit)) {
        unloadCudaPlugin();
        throw std::runtime_error("CUDA plugin initialization crashed (incompatible xmrig-cuda.dll?). Remove it from the exe folder for embedded Verus, or install a matching plugin build.");
    }
#   else
    pInit();
#   endif
