    ...
    void init(const CudaConfig &cuda)
    {
        ...
        if (CudaLib::init(cuda.loader())) {
            ...
            if (runtimeVersion && driverVersion && CudaLib::deviceCount()) {
                ...
                pluginReady = !devices.empty();
            }
        }
        else {
            Log::print(GREEN_BOLD(" * ") WHITE_BOLD("%-13s") RED_BOLD("plugin ") RED("(%s)"), kLabel, CudaLib::lastError());
        }

#       ifdef XMRIG_FEATURE_CUDA_VERUS
        if (devices.empty()) {
            auto embedded = CudaDevice::enumerateEmbeddedCudaDevices(cuda.bfactor(), cuda.bsleep());
            for (auto &d : embedded) {
                if (d.isValid()) {
                    devices.emplace_back(std::move(d));
                }
            }
            if (!devices.empty() && !pluginReady) {
                Log::print(GREEN_BOLD(" * ") WHITE_BOLD("%-13s") WHITE_BOLD("embedded Verus") BLACK_BOLD(" (CUDA runtime, no xmrig-cuda plugin)"), kLabel);
                controller->config()->generateCudaEmbeddedVerus(devices);
            }
        }
#       endif

        if (devices.empty()) {
#           if defined(XMRIG_ALGO_VERUSHASH) && !defined(XMRIG_FEATURE_CUDA_VERUS)
            Log::print(GREEN_BOLD(" * ") WHITE_BOLD("%-13s") YELLOW("Verus GPU needs a ") WHITE_BOLD("CUDA+nvcc rebuild") YELLOW(" of this miner or ") WHITE_BOLD("xmrig-cuda.dll") YELLOW(" — this exe has no embedded Verus kernel."),
                       kLabel);
#           elif defined(XMRIG_FEATURE_CUDA_VERUS)
            Log::print(GREEN_BOLD(" * ") WHITE_BOLD("%-13s") YELLOW("0 GPUs via CUDA runtime — check NVIDIA driver/GPU; keep ") WHITE_BOLD("cudart64_*.dll") YELLOW(" next to the exe."),
                       kLabel);
#           endif
...
#       ifdef XMRIG_FEATURE_NVML
        ...
        /* NVML + embedded-only CUDA (no xmrig-cuda plugin) has triggered heap corruption (0xC0000374) on some drivers; skip assign/init in that mode. */
        else if (!pluginReady) {
            printDisabled(kNvmlLabel, RED_S " (skipped for embedded CUDA Verus)");
        }
        else if (NvmlLib::init(cuda.nvmlLoader())) {
...
{
    return d_ptr->controller->config()->cuda().isEnabled() && !d_ptr->devices.empty();
}
